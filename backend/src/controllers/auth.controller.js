...
exports.register = async (req, res, next) => {
  try {
    ...
    if (!email || !username || !password || !role) return res.status(400).json({ message: "Field wajib diisi" });
...
exports.login = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id: row.id, role: row.role }, env.JWT_SECRET, { expiresIn: "7d" });
    return res.json({ message: "Login berhasil", token, user: row });
  } catch (err) { next(err); }
};

// ✅ WAJIB ADA AGAR TIDAK CRASH
